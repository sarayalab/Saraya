...
import { reactQuestionsNilaiUang } from "./../../../config/QuestionsNilaiUang";
import { getAuth } from "firebase/auth";
import { doc, getDoc, setDoc, updateDoc } from "firebase/firestore";
import { db } from "./../../../../firebase";
import { Ionicons } from "@expo/vector-icons";

const NilaiUang = ({ navigation }) => {
  ...
  const handleOptionPress = (pressedOption) => {
    ...
    const isAnswerCorrect =
      reactQuestionsNilaiUang[currentQuestionIndex].correctAnswer ===
      pressedOption;
  ...
  const handleNext = async () => {
    ...
    if (currentQuestionIndex === reactQuestionsNilaiUang.length - 1) {
      if (incorrectQuestions.length > 0) {
        ...
        updateCorrectAnswers(reactQuestionsNilaiUang.length);

        // Tandai modul sebagai selesai
        await completeModule(5); // ID modul untuk "Tujuan Keuangan"
    ...
    <SafeAreaView style={tw`flex-1`}>
      <ScrollView style={tw`p-4`}>
        <Text style={tw`text-base font-bold text-center mt-4 mb-5`}>
          {reactQuestionsNilaiUang[currentQuestionIndex].question}
        </Text>

        {reactQuestionsNilaiUang[currentQuestionIndex].options.map((option) => (
          <Pressable
            key={option}
            style={[
        ...
        >
          <Text style={tw`text-white text-center text-base`}>
            {currentQuestionIndex === reactQuestionsNilaiUang.length - 1
              ? "Finish"
              : "Next"}
          </Text>
        </Pressable>
