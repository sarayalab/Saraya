...
import { reactQuestionsAsetdanLiabilitas } from "./../../../config/QuestionsAsetdanLiabilitas";
import { getAuth } from "firebase/auth";
import { doc, getDoc, setDoc, updateDoc } from "firebase/firestore";
import { db } from "./../../../../firebase";
import { Ionicons } from "@expo/vector-icons";

const AsetdanLiabilitas = ({ navigation }) => {
  ...
  const handleOptionPress = (pressedOption) => {
    ...
    const isAnswerCorrect =
      reactQuestionsAsetdanLiabilitas[currentQuestionIndex].correctAnswer ===
      pressedOption;
  ...
  const handleNext = async () => {
    ...
    if (currentQuestionIndex === reactQuestionsAsetdanLiabilitas.length - 1) {
      if (incorrectQuestions.length > 0) {
        ...
        await completeModule(6); // ID modul untuk "Tujuan Keuangan"
        updateCorrectAnswers(reactQuestionsAsetdanLiabilitas.length);
    ...
    <SafeAreaView style={tw`flex-1`}>
      <ScrollView style={tw`p-4`}>
        <Text style={tw`text-base font-bold text-center mt-4 mb-5`}>
          {reactQuestionsAsetdanLiabilitas[currentQuestionIndex].question}
        </Text>

        {reactQuestionsAsetdanLiabilitas[currentQuestionIndex].options.map(
          (option) => (
            <Pressable
              key={option}
              style={[
        ...
        {selectedOption !== null && !isCorrect && (
          <Text style={tw`text-red-500 mt-2`}>
            Jawaban benar:{" "}
            {
              reactQuestionsAsetdanLiabilitas[currentQuestionIndex]
                .correctAnswer
            }
          </Text>
        )}
        ...
        >
          <Text style={tw`text-white text-center text-base`}>
            {currentQuestionIndex === reactQuestionsAsetdanLiabilitas.length - 1
              ? "Finish"
              : "Next"}
          </Text>
        </Pressable>
...
export default AsetdanLiabilitas;
