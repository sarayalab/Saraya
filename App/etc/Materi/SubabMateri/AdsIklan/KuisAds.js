...
const KuisAds = ({ navigation }) => {
  ...
  const handleNext = () => {
    if (currentQuestionIndex === reactQuestionsAds.length - 1) {
      const finalScore = calculateFinalScore(
        reactQuestionsAds.length,
        mistakes
      );
      updateCorrectAnswers(reactQuestionsAds.length);
      saveQuizScore(userUID, "quizAdsIklan", finalScore)
        .then(() => {
          return saveTimeSpent(userUID, "quizAdsIklan", timeElapsed);
        })
        .then(() => {
          navigation.navigate("scoreAdsIklan", {
            score: finalScore,
  ...
  const handleOptionPress = (pressedOption) => {
    ...
    const isAnswerCorrect =
      reactQuestionsAds[currentQuestionIndex].correctAnswer === pressedOption;
    ...
    <SafeAreaView style={tw`flex-1`}>
      ...
      <ScrollView style={tw`p-4`}>
        ...
        <Text style={tw`text-base font-bold text-center mt-4 mb-5`}>
          {reactQuestionsAds[currentQuestionIndex].question}
        </Text>

        {reactQuestionsAds[currentQuestionIndex].options.map((option) => (
          <Pressable
            key={option}
            style={[
        ...
        >
          <Text style={tw`text-white text-center text-base`}>
            {currentQuestionIndex === reactQuestionsAds.length - 1
              ? "Finish"
              : "Next"}
          </Text>
        </Pressable>
